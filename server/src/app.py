...
@app.route("/guess/<int:game>/<word>")
def guess(game, word):

    word_of_the_day = "apple"
    ...
    try:
        ...
        res = make_response("Unable to find word. Currently Lagom is running on a small dataset. Improvements coming soon!", 500)
        return res

    # magic fucking numbers
    cos = (np.dot(wotd_values,guess_values)/(norm(wotd_values)*norm(guess_values)) - 0.5) * 2
